...
COMPANIES = [
    ...
    },
    {
        ...
        "badges": [("live", "fa-circle", "Active"), ("a", "fa-microchip", "AI-Native Studio"), ("b", "fa-layer-group", "8 AI Ventures")],
        "stats": [("8", "AI Ventures"), ("6", "Industries"), ("3", "New Ventures / Year"), ("1", "Shared AI Stack")],
...
<header class="hero">
  ...
  <div class="hero-grid" data-scroll-parallax style="--depth:.55"></div>
...
"""
    return root + STATIC_CSS + PREMIUM_CSS + SCROLL_MOTION_CSS
...
# ── Scroll-driven motion (reference: motionsites.ai) ────────────────────────
# Distinct from `.reveal`, which is a one-shot IntersectionObserver toggle.
# These effects are scroll-LINKED: progress is a function of scroll position,
# so an element tracks the scrollbar and reverses when you scroll back up.
SCROLL_MOTION_CSS = r"""

/* ─── Scroll-linked motion primitives ─────────────────────────────────────
   Driven by a 0..1 --sp custom property. Native CSS scroll timelines handle
   the reveal case where the browser supports them (compositor-side, no JS);
   everything else reads --sp, which app.js sets in one batched rAF pass.
   Without JS, every element here renders in its final state.              */

body.js [data-scroll-reveal]{--sp:0;opacity:calc(.15 + .85 * var(--sp));transform:translateY(calc((1 - var(--sp)) * 28px))}

body.js [data-scroll-parallax]{--sp:.5;--depth:1;transform:translate3d(0,calc((var(--sp) - .5) * -80px * var(--depth)),0);will-change:transform}

body.js [data-scroll-scale]{--sp:0;transform:scale(calc(.94 + .06 * var(--sp)));will-change:transform}

/* A section that holds still while its inner track advances horizontally. */
.scroll-pin{position:relative}
.scroll-pin > .pin-stage{position:sticky;top:calc(var(--nav-h) + 16px);overflow:hidden}
.scroll-pin .pin-track{display:flex;gap:20px;transform:translate3d(calc(var(--sp,0) * -1 * var(--pin-travel,60%)),0,0);will-change:transform}

/* A bar that fills in step with how far through its container you have read. */
.read-progress{position:relative;height:3px;border-radius:3px;background:var(--border);overflow:hidden}
.read-progress::after{content:'';position:absolute;inset:0;transform-origin:0 50%;transform:scaleX(var(--sp,0));background:linear-gradient(90deg,var(--primary),var(--secondary))}

/* Where the browser has native scroll timelines, the reveal runs without JS
   touching it at all. app.js adds .has-scroll-timeline after feature-testing. */
@supports (animation-timeline: view()){
  body.js.has-scroll-timeline [data-scroll-reveal]{
    opacity:1;transform:none;
    animation:ss-scroll-rise linear both;
    animation-timeline:view();
    animation-range:entry 5% cover 35%;
  }
  @keyframes ss-scroll-rise{from{opacity:.15;transform:translateY(28px)}to{opacity:1;transform:none}}
}

@media(prefers-reduced-motion:reduce){
  body.js [data-scroll-reveal],
  body.js [data-scroll-parallax],
  body.js [data-scroll-scale]{opacity:1!important;transform:none!important;animation:none!important}
  .scroll-pin .pin-track{transform:none!important}
  .read-progress::after{transform:scaleX(1)!important}
}
"""


# ── Scroll-driven motion JS (appended to APP_JS for every site) ─────────────
SCROLL_MOTION_JS = r"""

/* ─── Scroll-linked motion ──────────────────────────────────────────────────
   Sets a 0..1 --sp custom property on opted-in elements from their position in
   the viewport, batched into a single rAF pass per scroll event. Complements
   the one-shot .reveal observer above: these effects follow the scrollbar in
   both directions rather than firing once. */
(function(){
  if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

  // Native scroll timelines cover the reveal case on the compositor, so when
  // they exist we leave reveals alone and only drive the rest from JS.
  var nativeTimeline = (typeof CSS !== 'undefined' && CSS.supports &&
                        CSS.supports('animation-timeline: view()'));
  if (nativeTimeline) document.body.classList.add('has-scroll-timeline');

  var SEL = '[data-scroll-parallax],[data-scroll-scale],.scroll-pin,.read-progress' +
            (nativeTimeline ? '' : ',[data-scroll-reveal]');

  var items = [];
  function collect(){ items = [].slice.call(document.querySelectorAll(SEL)); }
  ...
  function update(){
    ...
    for (var i = 0; i < items.length; i++){
      var el = items[i];
      var r = el.getBoundingClientRect();
      // 0 when the element's top edge sits one viewport below the fold,
      // 1 once its bottom edge has travelled past the top of the viewport.
  ...
  window.addEventListener('resize', function(){ collect(); update(); }, { passive: true });
})();
...
def main():
    for c in COMPANIES:
        ...
        with open(os.path.join(folder, "app.js"), "w", encoding="utf-8") as f:
            f.write(APP_JS + SCROLL_MOTION_JS)
