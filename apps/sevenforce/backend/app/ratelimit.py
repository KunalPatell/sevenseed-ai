...
(e.g. the landing-page "Try Maya" AI demo widget). Single-process only — this
app runs uvicorn with no --workers, so an in-memory dict is authoritative.
Resets on restart; won't coordinate across multiple instances if ever
horizontally scaled, which is an acceptable limitation for a single-dyno
deployment. Adapted from apps/sevenseed/backend/app/ratelimit.py (Phase 1).
"""
