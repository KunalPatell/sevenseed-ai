  ...
  const onContact = async (e: React.FormEvent) => {
    ...
    try {
      await submitContact({
        name: contactName,
        email: contactEmail,
        subject: "Construction project request",
        message: contactMsg,
        website: honeypot,
      });
      setContactStatus("sent");
      setFeedbackMsg("Message received! Our team replies within 1 business day.");
        ...
        <div className="relative z-10 w-full max-w-[var(--maxw)] mx-auto px-6 md:px-12 py-16 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          >
            <div className="relative flex items-center gap-4">
              <PersonaAvatar size={210} primary="#f59e0b" secondary="#fcd34d" accessory="helmet" name="Argus" role="Site Safety AI" />
              <div className="hidden xl:block relative rounded-2xl rounded-bl-none border border-[rgba(245,158,11,0.3)] bg-[rgba(9,9,20,0.85)] px-4 py-3 text-sm text-[#cbd5e1] max-w-[200px] backdrop-blur">
                <span className="text-[#fcd34d] font-semibold">Hi, I&apos;m Argus 👋</span><br />
                I scan your site for safety risks &amp; defects, live.
              </div>
            </div>
            <DefectScannerVisual />
          </motion.div>
          ...
          <GlowCard className="glow-card bg-[#0d0a06] border border-white/5 rounded-2xl p-10 relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-[560px] mx-auto text-center">
              ...
              <form onSubmit={onContact} className="flex flex-col gap-3 text-left">
                ...
                >
                  {contactStatus === "sending" ? "Sending…" : "Submit Project Request"}
                </button>
                {feedbackMsg && (
                  <p
                    role="status"
