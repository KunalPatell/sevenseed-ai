...
# Resolve absolute path to decode-forest-pharmacy backend directory
HERE = Path(__file__).resolve()
APPS_DIR = HERE.parents[4] / "apps"
PHARMACY_DIR = APPS_DIR / "decode-forest-pharmacy" / "backend"
...
spec = importlib.util.spec_from_file_location("pharmacy_main", str(PHARMACY_DIR / "main.py"))
pharmacy_main = importlib.util.module_from_spec(spec)
sys.modules["pharmacy_main"] = pharmacy_main
spec.loader.exec_module(pharmacy_main)
...
router = APIRouter()
for route in pharmacy_main.app.routes:
