  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 h-[66px] z-[100] flex items-center justify-between px-6 md:px-12 backdrop-blur-md transition-all duration-300 border-b border-transparent ${
      ...
      <Link href="/" className="flex items-center gap-3 font-extrabold text-[17px] tracking-tight">
        ...
        <span className="text-white whitespace-nowrap">Seven<span className="text-[#6ee7b7]">seed</span></span>
      </Link>
      ...
      <div className={`absolute top-[66px] left-0 right-0 bg-[#0d0d16] backdrop-blur-xl shadow-2xl border-b border-white/5 p-6 flex flex-col gap-4 md:static md:flex md:flex-row md:bg-transparent md:border-none md:p-0 md:gap-1 transition-all duration-300 ${
        ...
        <a href="#portfolio" onClick={() => setMenuOpen(false)} className="px-4 py-2 text-sm text-[#9aa0b8] hover:text-[#eeeef8] hover:bg-white/[0.04] rounded-lg transition-all duration-200">Portfolio</a>
        <a href="#ideate" onClick={() => setMenuOpen(false)} className="px-4 py-2 text-sm text-[#9aa0b8] hover:text-[#eeeef8] hover:bg-white/[0.04] rounded-lg transition-all duration-200">AI Ideation</a>
        ...
        <Link href="/app/" onClick={() => setMenuOpen(false)} className="md:hidden px-4 py-2 text-sm font-semibold text-white bg-white/[0.06] border border-white/10 rounded-lg text-center">Studio Hub</Link>
      </div>

      <div className="flex items-center gap-3">
        <Link href="/app/" className="hidden md:inline-flex btn bg-gradient-to-r from-[#8b5cf6] to-[#10b981] hover:scale-[1.02] text-white text-xs md:text-sm font-semibold px-4 py-2.5 rounded-lg transition-all duration-200 shadow-[0_6px_18px_rgba(139,92,246,0.25)]">
          <i className="fas fa-rocket mr-1.5"></i> Studio Hub
        </Link>
