"use client";

import React from "react";
import Link from "next/link";
import { Leaf } from "lucide-react";

export function Footer() {
  const year = new Date().getFullYear();

  return (
    <footer className="border-t border-white/5 bg-[#050508] py-16 px-6 md:px-12 mt-20">
      <div className="max-w-[1180px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-10">
        <div className="flex flex-col gap-4">
          <Link href="/" className="flex items-center gap-3 font-extrabold text-[17px] tracking-tight">
            <span className="w-[36px] h-[36px] rounded-[11px] grid place-items-center text-white bg-gradient-to-br from-[#8b5cf6] to-[#10b981] shadow-[0_6px_16px_rgba(139,92,246,0.3)]">
              <Leaf className="h-[18px] w-[18px]" />
            </span>
            <span className="text-white">Seven<span className="text-[#6ee7b7]">seed</span></span>
          </Link>
          <p className="text-xs md:text-sm text-[#9aa0b8] leading-relaxed">
            Sevenseed is an AI-native venture studio that ideates, incubates, and launches market-leading AI software startups in India.
          </p>
          <div className="flex gap-4 text-[#9aa0b8]">
            <a href="#" className="hover:text-[#8b5cf6] transition-all"><i className="fab fa-linkedin-in text-lg"></i></a>
            <a href="#" className="hover:text-[#8b5cf6] transition-all"><i className="fab fa-instagram text-lg"></i></a>
            <a href="#" className="hover:text-[#8b5cf6] transition-all"><i className="fab fa-facebook-f text-lg"></i></a>
            <a href="mailto:hello@sevenseed.in" className="hover:text-[#8b5cf6] transition-all"><i className="fas fa-envelope text-lg"></i></a>
          </div>
        </div>

        <div className="flex flex-col gap-4">
          <h5 className="font-semibold text-sm text-white uppercase tracking-wider">Explore</h5>
          <ul className="flex flex-col gap-2.5 text-sm text-[#9aa0b8]">
            <li><a href="#about" className="hover:text-[#eeeef8] transition-all">About Studio</a></li>
            <li><a href="#portfolio" className="hover:text-[#eeeef8] transition-all">Startups Portfolio</a></li>
            <li><a href="#ideate" className="hover:text-[#eeeef8] transition-all">Venture Ideation</a></li>
            <li><a href="#faq" className="hover:text-[#eeeef8] transition-all">FAQ</a></li>
            <li><a href="#contact" className="hover:text-[#eeeef8] transition-all">Contact Us</a></li>
          </ul>
        </div>

        <div className="flex flex-col gap-4">
          <h5 className="font-semibold text-sm text-white uppercase tracking-wider">Startups Portfolio</h5>
          <ul className="flex flex-col gap-2.5 text-sm text-[#9aa0b8]">
            <li><Link href="/comonk-ai" className="hover:text-[#eeeef8] transition-all">Comonk Technology</Link></li>
            <li><Link href="/avpu" className="hover:text-[#eeeef8] transition-all">AVP University</Link></li>
            <li><Link href="/avp-emart" className="hover:text-[#eeeef8] transition-all">AVP Emart</Link></li>
            <li><Link href="/pharmacy" className="hover:text-[#eeeef8] transition-all">Decode Forest Pharmacy</Link></li>
            <li><Link href="/breakdown" className="hover:text-[#eeeef8] transition-all">Breakdown Factor</Link></li>
            <li><Link href="/trust" className="hover:text-[#eeeef8] transition-all">AVP Charitable Trust</Link></li>
            <li><Link href="/sevenforce" className="hover:text-[#eeeef8] transition-all">Sevenforce</Link></li>
          </ul>
        </div>

        <div className="flex flex-col gap-4">
          <h5 className="font-semibold text-sm text-white uppercase tracking-wider">Contact</h5>
          <ul className="flex flex-col gap-3 text-sm text-[#9aa0b8]">
            <li className="flex items-center gap-3"><i className="fas fa-envelope text-[#8b5cf6]"></i> hello@sevenseed.in</li>
            <li className="flex items-center gap-3"><i className="fas fa-phone text-[#8b5cf6]"></i> +91 84908 61586</li>
            <li className="flex items-center gap-3"><i className="fas fa-location-dot text-[#8b5cf6]"></i> Ahmedabad, Gujarat, India</li>
          </ul>
        </div>
      </div>

      <div className="max-w-[1180px] mx-auto border-t border-white/5 mt-12 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs md:text-sm text-[#5b5f78]">
        <span>© {year} Sevenseed Venture Studio. All rights reserved.</span>
        <span className="flex items-center gap-1.5"><i className="fas fa-seedling text-[#8b5cf6]"></i> Incubating Tomorrow</span>
      </div>
    </footer>
  );
}
