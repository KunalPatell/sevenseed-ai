...
export default function AppPortal() {
  ...
  useEffect(() => {
    ...
    window.fetch = async (input, init) => {
      ...
      if (path.includes("/api/")) {
        ...
        // Session token, attached in the same place as the BYOK keys. The
        // endpoints returning a person's wishlist, alerts, searches and orders
        // require it now — they used to be readable by anyone with the URL.
        const session = localStorage.getItem(EMART_TOKEN_KEY);
  ...
  // Sign-in state. The backend has always had /api/auth/signup, /login and /me;
  // this portal never used them, which is why the endpoints returning a shopper's
  // wishlist, alerts, searches and orders had to be left open. They are closed now.
  ...
  // Restore a session on load and confirm the token is still good.
  useEffect(() => {
    const saved = typeof window !== "undefined" ? localStorage.getItem(EMART_TOKEN_KEY) : null;
    if (!saved) return;
    setAuthToken(saved);
    (async () => {
      try {
  ...
  const handleAuthSubmit = async () => {
    ...
    try {
      ...
      if (d?.error || !d?.token) {
        ...
        localStorage.setItem(EMART_TOKEN_KEY, d.token);
  ...
  const handleSignOut = () => {
    localStorage.removeItem(EMART_TOKEN_KEY);
    setAuthToken("");
    setAuthUser(null);
  };
      ...
      <div className="flex flex-col gap-6 animate-[fade_0.3s_ease]">
        ...
        {compareLoading ? (
          ...
          <div className="flex flex-col gap-6">
            ...
            {(() => {
              const prices = compareResults.map(p => p.price).filter(p => p > 0);
              const maxP = Math.max(...prices);
              const minP = Math.min(...prices);
              const savings = maxP - minP;
              const pct = maxP > 0 ? (savings / maxP) * 100 : 0;
              const cheapest = compareResults.find(p => p.price === minP) || compareResults[0];
              ...
              const downloadReport = () => {
                ...
                compareResults.forEach(r => {
                  rowsHtml += `
                    <tr>
                      ...
                      <td>${r.best_value_score || 0}</td>
                    </tr>`;
                });
                const htmlContent = `<!DOCTYPE html>
                <html><head><meta charset="UTF-8"><title>Price Comparison — ${compareQuery}</title>
                <style>
                  body{font-family:system-ui,sans-serif;background:#0f172a;color:#e2e8f0;padding:2rem;}
                  h1{color:#4ade80;} h2{color:#38bdf8;border-bottom:1px solid #334155;padding-bottom:0.4rem;}
                  table{width:100%;border-collapse:collapse;margin-top:1rem;}
                  th{background:#1e293b;color:#94a3b8;padding:10px;text-align:left;font-size:0.8rem;text-transform:uppercase;}
                  td{padding:10px;border-bottom:1px solid #1e293b;font-size:0.88rem;}
                  tr:hover{background:#1e293b;}
                  .rec{background:#064e3b;border:1px solid #10b981;border-radius:12px;padding:1.5rem;margin:1rem 0;}
                  .footer{color:#475569;font-size:0.78rem;margin-top:2rem;}
                </style></head>
                <body>
                <h1>🛒 Price Comparator Pro</h1>
                <p>Search: <strong>${compareQuery}</strong> | Generated: ${new Date().toLocaleString()}</p>
                <div class="rec">
                  <h2>🏆 Best Recommendation</h2>
                  <p><strong>${compareResults[0].title}</strong></p>
                  <p>Platform: ${compareResults[0].platform} | Price: ₹${compareResults[0].price} | Rating: ${compareResults[0].rating}⭐ | Value Score: ${compareResults[0].best_value_score || 0}</p>
                  <a href="${compareResults[0].url}" style="color:#6ee7b7;">Buy Now →</a>
                </div>
                <h2>📋 Full Comparison (${compareResults.length} products)</h2>
                <table><tr><th>Product</th><th>Site</th><th>Price</th><th>Rating</th><th>Reviews</th><th>Score</th></tr>
                ${rowsHtml}</table>
                <p class="footer">Generated by AVP Emart • Powered by SerpAPI & Next.js</p>
                </body></html>`;
                ...
                a.href = url;
                a.download = `price_report_${compareQuery.replace(/[^A-Za-z0-9_.]/g, '_')}.html`;
              ...
              return (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="bg-gradient-to-br from-[#1e1b4b]/40 to-[#312e81]/15 border border-[#818cf8]/30 rounded-2xl p-5 flex flex-col justify-between">
                    <div>
                      <span className="text-[10px] uppercase font-bold text-[#818cf8] tracking-wider">💸 Savings Calculator</span>
                      <div className="text-3xl font-black text-[#a5b4fc] mt-2">₹{savings.toLocaleString()}</div>
                      <div className="text-xs text-[#818cf8] mt-1">({pct.toFixed(1)}% off highest price)</div>
                    </div>
                    <button onClick={downloadReport} className="btn bg-[#312e81] hover:bg-[#1e1b4b] text-white border border-[#818cf8]/40 text-xs py-2 px-3 rounded-xl flex items-center justify-center gap-1.5 cursor-pointer mt-4 w-full">
                      <i className="fas fa-download"></i> Export HTML Report
                    </button>
                  </div>
                  <div className="bg-[#0d0f0e] border border-white/5 rounded-2xl p-5 flex flex-col justify-between">
                    <div>
                      <span className="text-[10px] uppercase font-bold text-[#8890aa] tracking-wider">Most Expensive Match</span>
                      <div className="text-xl font-bold text-white mt-2">₹{maxP.toLocaleString()}</div>
                      <div className="text-[10px] text-[#8890aa] mt-1">Avoid overpaying by scanning all sites</div>
                    </div>
                  </div>
                  <div className="bg-[#0d0f0e] border border-white/5 rounded-2xl p-5 flex flex-col justify-between">
                    <div>
                      <span className="text-[10px] uppercase font-bold text-[#10b981] tracking-wider">Best Platform Deal</span>
                      <div className="text-xl font-bold text-white mt-2">₹{minP.toLocaleString()}</div>
                      <div className="text-[10px] text-[#6ee7b7] mt-1">on {cheapest.platform}</div>
                    </div>
                  </div>
                </div>
              );
            ...
            {(() => {
              ...
              compareResults.forEach(p => {
                if (!siteBest[p.platform] || (p.best_value_score || 0) > (siteBest[p.platform].best_value_score || 0)) {
                  siteBest[p.platform] = p;
                }
              });
              const icons: { [key: string]: string } = {
                "Amazon": "🟠", "Flipkart": "🔵", "Reliance Digital": "🔴", "Snapdeal": "🟡"
              };
              return (
                <div className="flex flex-col gap-3">
                  <h4 className="font-extrabold text-[#eeeef8] text-xs uppercase tracking-wider">🏪 Per-Site Best Deals</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {Object.entries(siteBest).map(([platform, item]) => (
                      <div key={platform} className="bg-[#12121e] border border-white/5 rounded-xl p-4 flex flex-col justify-between gap-3 hover:border-white/10 transition-all">
                        <div>
                          <div className="text-xs font-extrabold text-white flex items-center gap-1.5">
                            <span>{icons[platform] || "🏪"}</span> {platform}
                          </div>
                          <div className="text-sm font-bold text-[#6ee7b7] mt-1">₹{item.price.toLocaleString()}</div>
                          <div className="text-[10px] text-[#8890aa] mt-1 truncate">{item.title}</div>
                        </div>
                        <a href={item.url} target="_blank" rel="noopener noreferrer" className="btn bg-white/5 border border-white/10 text-white text-[10px] hover:bg-white/10 py-1.5 px-2.5 rounded-lg flex items-center justify-center gap-1">
        ...
        >
          ...
          >
            <div>
              ...
              <p className="text-xs text-[#8890aa] mt-1.5 leading-relaxed">
                Your wishlist, price alerts and past searches are private to your account.
                Comparing prices works without signing in.
              </p>
            </div>
            ...
            {authMode === "signup" && (
              <input
                ...
                className="w-full bg-[#12121e] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#ea580c]/50"
              />
            )}
            <input
            ...
            <button
              ...
              className="btn w-full bg-gradient-to-r from-[#ea580c] to-[#f97316] text-white font-semibold py-3 rounded-xl cursor-pointer disabled:opacity-60"
            >
            ...
            <button
              ...
              className="text-xs text-[#8890aa] hover:text-white transition-colors cursor-pointer"
            >
