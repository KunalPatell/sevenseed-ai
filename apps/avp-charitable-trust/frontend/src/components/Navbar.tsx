  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 h-[66px] z-[100] flex items-center justify-between px-6 md:px-12 backdrop-blur-md transition-all duration-300 border-b border-transparent ${
      ...
      <Link href="/" className="flex items-center gap-3 font-extrabold text-[17px] tracking-tight">
        ...
        <span className="text-white whitespace-nowrap">AVP<span className="text-[#f43f5e]">Trust</span></span>
      </Link>

      <div className={`absolute top-[66px] left-0 right-0 bg-[#10080a] backdrop-blur-xl shadow-2xl border-b border-white/5 p-6 flex flex-col gap-4 md:static md:flex md:flex-row md:bg-transparent md:border-none md:p-0 md:gap-1 transition-all duration-300 ${
        ...
        <Link href="/app/" onClick={() => setMenuOpen(false)} className="md:hidden px-4 py-2 text-sm font-semibold text-white bg-white/[0.06] border border-white/10 rounded-lg text-center">NGO Portal</Link>
      </div>

      <div className="flex items-center gap-3">
        <Link href="/app/" className="hidden md:inline-flex btn bg-gradient-to-r from-[#f43f5e] to-[#f59e0b] hover:scale-[1.02] text-white text-xs md:text-sm font-semibold px-4 py-2.5 rounded-lg transition-all duration-200 shadow-[0_6px_18px_rgba(244,63,94,0.25)]">
