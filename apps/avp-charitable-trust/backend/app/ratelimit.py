...
(the AI ideation demo, the contact form). Single-process only — this app runs
uvicorn with no --workers, so an in-memory dict is authoritative. Resets on
restart; won't coordinate across multiple instances if ever horizontally
scaled, which is an acceptable limitation for a single-dyno deployment.
"""
