# -*- coding: utf-8 -*-
"""
Breakdown Factor — AI Construction Engineering & Safety Workstation.
Engineered with features from:
- CPWD DSR 2023 Civil Schedules (Instant BOQ Estimator & Materials Calculator)
- Procore / OSHA Compliance (Standardized Safety Audit & CAPA Action Generator)
- OpenSpace.ai (Computer Vision PPE & Site Hazard Detection)
- 21st.dev (Conic Animated Border Beams & Bento Grids)
- Unicorn Studio (Liquid Fluid Shader Background)
- Aceternity UI (Overhead Lamp Illumination & 3D Card Tilt)
"""

def render_breakdown_html(c):
    email, phone, location = c["contact"]
    
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Breakdown Factor — AI Construction Safety, BOQ Estimator & CV Scanner</title>
  <meta name="description" content="AI-driven civil engineering. CPWD DSR 2023 BOQ cost forecasting, OSHA safety compliance audits, and real-time computer vision PPE site monitoring.">
  <meta property="og:type" content="website">
  <meta property="og:title" content="Breakdown Factor — AI Construction Safety">
  <meta property="og:description" content="CPWD BOQ civil estimation and YOLO computer vision PPE monitoring.">
  <meta property="og:url" content="https://sevenseed.onrender.com/breakdown/">
  <link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Ctext y='.9em' font-size='88'%3E🏗️%3C/text%3E%3C/svg%3E">
  <script>(function(){{try{{var t=localStorage.getItem('ss-theme')||'dark';document.documentElement.setAttribute('data-theme',t);}}catch(e){{}}}})();</script>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&family=JetBrains+Mono:wght@400;500;600&family=Outfit:wght@400;500;600;700;800;900&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
  <link rel="stylesheet" href="style.css">
  <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>
</head>
<body data-variant="split-screen" style="--font-display:'Outfit', sans-serif;">
<span id="top"></span>

<div class="preloader" id="preloader">
  <div class="pl-glow pl-glow-1"></div>
  <div class="pl-glow pl-glow-2"></div>
  <div class="pl-content">
    <div class="pl-logo">
      <div class="pl-icon-wrap"><i class="fas fa-helmet-safety pl-icon"></i><span class="pl-ring"></span></div>
      <div class="pl-name">BREAKDOWN FACTOR</div>
    </div>
    <div class="pl-progress">
      <div class="pl-bar-track"><div class="pl-bar" id="plBar"></div></div>
      <div class="pl-text">CONNECTING CV SENSORS & BOQ SCHEDULES… <span id="plPct">0</span>%</div>
    </div>
  </div>
</div>
<div class="grain" aria-hidden="true"></div>
<div class="scroll-progress" id="scrollProgress"></div>
<div class="cursor-ring" id="cursorRing" aria-hidden="true"></div>

<!-- Top Navigation -->
<nav class="nav">
  <a class="logo" href="#top">
    <span class="logo-icon"><i class="fas fa-helmet-safety"></i></span>
    <span class="logo-text">Breakdown <span class="logo-accent">Factor</span></span>
  </a>
  <div class="nav-links" id="navLinks">
    <a href="boq-estimator.html"><i class="fas fa-calculator" style="color:#fbbf24;"></i> BOQ Estimator</a>
    <a href="safety-audit.html"><i class="fas fa-clipboard-check" style="color:#34d399;"></i> Safety Audit</a>
    <a href="cv-scanner.html"><i class="fas fa-camera" style="color:#38bdf8;"></i> CV Site Scanner</a>
    <a href="#workstations">Workstations</a>
    <a href="#faq">FAQ</a>
  </div>
  <div class="nav-right">
    <button class="icon-btn" id="searchBtn" type="button" aria-label="Search (Ctrl+K)" title="Search (Ctrl+K)"><i class="fas fa-magnifying-glass"></i></button>
    <button class="icon-btn" id="themeToggle" type="button" aria-label="Toggle light / dark theme" title="Toggle theme"><i class="fas fa-moon"></i></button>
    <a class="btn btn-ghost" href="boq-estimator.html"><i class="fas fa-calculator"></i> Calculate BOQ</a>
    <a class="btn btn-primary" href="cv-scanner.html"><i class="fas fa-video"></i> Scan Site</a>
    <button class="hamburger" id="hamburger" aria-label="Menu"><i class="fas fa-bars"></i></button>
  </div>
</nav>

<!-- Hero Section with Overhead Lamp & 3D Construction Scaffold -->
<header class="hero">
  <div class="hero-lamp"></div>
  <div class="hero-lamp-line"></div>
  <div class="liquid-mesh"></div>
  <div class="meteors-container">
    <span class="meteor" style="--top:14%; --left:24%; --delay:0s; --duration:4.1s;"></span>
    <span class="meteor" style="--top:28%; --left:62%; --delay:1.6s; --duration:5.3s;"></span>
  </div>
  <div class="hero-glow"></div>
  <div class="hero-grid"></div>

  <div class="hero-content">
    <div class="hero-pill" data-blur-in style="--i:0">
      <i class="fas fa-helmet-safety"></i> <span>AI Civil Engineering · CPWD DSR 2023 · 100% Free</span>
    </div>
    <h1 class="hero-title" data-blur-in style="--i:1">
      AI-Driven Construction.<br><span class="grad">Zero Accidents</span> & Precision Costing.
    </h1>
    <p class="hero-sub" data-blur-in style="--i:2">
      Engineered with CPWD DSR 2023 civil schedules and OSHA standards. Generate instant Bill of Quantities (BOQ) with contractor profit and GST calculations, run standardized safety audits, and scan sites using YOLO computer vision.
    </p>

    <div class="hero-actions" data-blur-in style="--i:3">
      <a class="btn btn-primary lg" href="boq-estimator.html"><i class="fas fa-calculator"></i> Launch CPWD BOQ Estimator →</a>
      <a class="btn btn-ghost lg" href="safety-audit.html"><i class="fas fa-clipboard-check"></i> Run Safety Audit</a>
      <a class="btn btn-ghost lg" href="cv-scanner.html"><i class="fas fa-camera"></i> Launch CV Scanner</a>
    </div>

    <div class="stats-row" data-blur-in style="--i:4">
      <div class="stat"><span class="stat-num">CPWD 2023</span><span class="stat-lbl">Standard Rates</span></div>
      <div class="stat"><span class="stat-num">28ms</span><span class="stat-lbl">YOLO Vision Latency</span></div>
      <div class="stat"><span class="stat-num">100%</span><span class="stat-lbl">Free Tools</span></div>
      <div class="stat"><span class="stat-num">ISO 45001</span><span class="stat-lbl">Safety Aligned</span></div>
    </div>

    <div class="hero-marquee" data-blur-in style="--i:5">
      <div class="marquee-track">
        <span>CPWD DSR 2023 BOQ</span><span>OSHA Safety Audits</span><span>YOLO PPE Scanner</span><span>RCC Cement & Steel Calculations</span><span>Defect Crack Detection</span>
        <span>CPWD DSR 2023 BOQ</span><span>OSHA Safety Audits</span><span>YOLO PPE Scanner</span><span>RCC Cement & Steel Calculations</span><span>Defect Crack Detection</span>
      </div>
    </div>
  </div>

  <!-- 3D PBR WebGL Interactive Stage -->
  <div class="hero-3d-stage" id="hero3dStage">
    <canvas id="hero3dCanvas"></canvas>
    <div class="aura-telemetry left"><span class="aura-dot"></span> 120 FPS WebGL · Cyber Scaffold</div>
    <div class="aura-telemetry right"><i class="fas fa-arrows-spin"></i> 360° Drag & Orbit</div>
  </div>
</header>

<!-- Bento Workstations -->
<section class="section" id="workstations" style="padding-top:40px; padding-bottom:60px;">
  <div class="sec-head reveal">
    <div class="eyebrow"><i class="fas fa-cubes"></i> Construction Engineering Workstations</div>
    <h2 class="sec-title">Interactive AEC & Civil Engines</h2>
    <p class="sec-sub">Built to CPWD DSR 2023 civil schedules, Procore OSHA compliance rubrics, and computer vision site monitoring standards.</p>
  </div>

  <div class="bento-showcase">
    <!-- 1. BOQ Estimator -->
    <div class="border-beam-card">
      <div>
        <div style="font-size:32px; margin-bottom:12px;">🏗️</div>
        <h3 style="font-size:18px; font-weight:700; color:#fff; margin-bottom:8px;">CPWD DSR 2023 BOQ Estimator</h3>
        <p style="font-size:13px; color:var(--text-2); line-height:1.6; margin-bottom:16px;">Civil construction bill-of-quantities takeoff with RCC M25, brickwork, plastering, 15% contractor profit, and 18% GST calculation.</p>
      </div>
      <a class="btn btn-primary sm" href="boq-estimator.html"><i class="fas fa-calculator"></i> Open BOQ Estimator →</a>
    </div>

    <!-- 2. Safety Audit -->
    <div class="border-beam-card">
      <div>
        <div style="font-size:32px; margin-bottom:12px;">🦺</div>
        <h3 style="font-size:18px; font-weight:700; color:#fff; margin-bottom:8px;">Procore OSHA Safety Audit</h3>
        <p style="font-size:13px; color:var(--text-2); line-height:1.6; margin-bottom:16px;">Standardized site safety inspection checklist, live safety compliance scoring, risk level tiering, and CAPA corrective action generator.</p>
      </div>
      <a class="btn btn-primary sm" href="safety-audit.html"><i class="fas fa-clipboard-check"></i> Run Safety Audit →</a>
    </div>

    <!-- 3. CV Scanner -->
    <div class="border-beam-card">
      <div>
        <div style="font-size:32px; margin-bottom:12px;">📷</div>
        <h3 style="font-size:18px; font-weight:700; color:#fff; margin-bottom:8px;">CV Site Safety Scanner</h3>
        <p style="font-size:13px; color:var(--text-2); line-height:1.6; margin-bottom:16px;">Live camera and image scanning for PPE compliance: hardhat detection, high-vis vests, harness attachment, and perimeter monitoring.</p>
      </div>
      <a class="btn btn-primary sm" href="cv-scanner.html"><i class="fas fa-camera"></i> Launch CV Scanner →</a>
    </div>
  </div>
</section>

<!-- Group Footer -->
<footer class="foot">
  <div class="foot-grid">
    <div class="foot-brand">
      <a class="logo" href="#top">
        <span class="logo-icon"><i class="fas fa-helmet-safety"></i></span>
        <span class="logo-text">Breakdown <span class="logo-accent">Factor</span></span>
      </a>
      <p>AI Construction Engineering, Site Safety Auditing, and Civil BOQ Forecaster. Part of Sevenseed AI Studio.</p>
    </div>
    <div class="foot-col">
      <h5>Civil Tools</h5>
      <ul>
        <li><a href="boq-estimator.html">CPWD BOQ Estimator</a></li>
        <li><a href="safety-audit.html">OSHA Safety Audit</a></li>
        <li><a href="cv-scanner.html">YOLO PPE Scanner</a></li>
        <li><a href="../index.html">Sevenseed Hub</a></li>
      </ul>
    </div>
    <div class="foot-col">
      <h5>Connect</h5>
      <ul>
        <li><span><i class="fas fa-envelope"></i> {email}</span></li>
        <li><span><i class="fas fa-location-dot"></i> {location}</span></li>
        <li><a href="https://github.com/KunalPatell/sevenseed-ai" target="_blank" rel="noopener"><i class="fab fa-github"></i> Public GitHub</a></li>
      </ul>
    </div>
  </div>
  <div class="foot-bottom">
    <div>© 2026 Breakdown Factor. 100% Free AI Civil Tech. Part of Sevenseed AI Studio.</div>
  </div>
</footer>

<script src="app.js"></script>
</body>
</html>"""
